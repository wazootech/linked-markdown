...
Subject: New CG work item: Linked Markdown Protocol (LMP) — JSON-LD frontmatter for typed Markdown documents
...
**Linked Markdown Protocol (LMP)** — a specification for structuring,
...
The core idea: an LMP document is simultaneously valid CommonMark and valid
JSON-LD. A single `id` and `@type` field in the frontmatter turns any
Markdown file into a typed RDF node. From there, LMP layers validation
...
into the authoring/document space, and LMP aims to continue that direction
